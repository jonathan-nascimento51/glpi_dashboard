...
class GLPIAdapter(GLPIRepositoryInterface):
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.glpi_url}/initSession", headers=headers, timeout=30
            )
    ...
    async def get_dashboard_metrics(self) -> DashboardMetrics:
        ...
        niveis = {
            "N1": LevelMetrics(
                novos=5, progresso=12, pendentes=3, resolvidos=45, total=65
            ),
            "N2": LevelMetrics(
                novos=8, progresso=15, pendentes=5, resolvidos=32, total=60
            ),
            "N3": LevelMetrics(
                novos=2, progresso=8, pendentes=1, resolvidos=18, total=29
            ),
            "N4": LevelMetrics(
                novos=1, progresso=3, pendentes=0, resolvidos=8, total=12
            ),
        }
