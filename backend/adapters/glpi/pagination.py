...
class PaginationParams(BaseModel):
    ...
    cursor: Optional[str] = Field(default=None, description="Cursor para paginação baseada em cursor")
    ...
    def to_glpi_criteria(self) -> List[Dict[str, Any]]:
        ...
        if self.priority:
            ...
            criteria.append({
                ...
                f"criteria[{criteria_index}][searchtype]": "equals",
                f"criteria[{criteria_index}][value]": priority_values[0] if len(priority_values) == 1 else priority_values
            })
            criteria_index += 1
        ...
        if self.date_range:
            ...
            if date_filters.get('start_date'):
                ...
                criteria.append({
                    ...
                    f"criteria[{criteria_index}][value]": date_filters['start_date']
                })
                criteria_index += 1
...
class SortParams(BaseModel):
    ...
    order: SortOrder = Field(default=SortOrder.DESC, description="Ordem de classificação")
