    ...
    try:
        if USE_REAL_GLPI_DATA:
            ...
            return ResponseFormatter.format_success_response(
                data=metrics, message="KPIs obtidos do GLPI"
            )
        else:
            mock_data = {
                "niveis": {
            ...
            return ResponseFormatter.format_success_response(
                data=mock_data, message="KPIs mockados"
            )
...
async def get_system_status():
    try:
        ...
        return ResponseFormatter.format_success_response(
            data=status_data, message="Status do sistema"
        )
    except Exception as e:
...
async def get_technician_ranking(limit: Optional[int] = Query(10)):
    try:
        if USE_REAL_GLPI_DATA:
            ...
            return ResponseFormatter.format_success_response(
                data=ranking_data, message="Ranking de tecnicos do GLPI"
            )
        else:
            ranking_data = [
                {
            ...
            return ResponseFormatter.format_success_response(
                data=ranking_data[:limit], message="Ranking de tecnicos"
            )
    except Exception as e:
...
async def get_new_tickets(limit: Optional[int] = Query(10)):
    try:
        if USE_REAL_GLPI_DATA:
            ...
            return ResponseFormatter.format_success_response(
                data=tickets_data, message="Novos tickets do GLPI"
            )
        else:
            mock_tickets = [
                {
            ...
            return ResponseFormatter.format_success_response(
                data=mock_tickets[:limit], message="Novos tickets mockados"
            )
    except Exception as e:
