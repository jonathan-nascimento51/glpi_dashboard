...
async def get_structured_logs(limit: Optional[int] = 100):
    ...
    try:
        ...
        return ApiResponse(
            success=True,
            data=logs,
            message=f"Retornados {len(logs)} eventos de log"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="Erro interno ao obter logs"
        )
...
async def get_metrics_health():
    ...
    try:
        ...
        return ApiResponse(
            success=False,
            data=health_data,
            message="Sistema de métricas com problemas"
        )
...
async def reset_metrics():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Erro interno ao resetar métricas"
        )