...
from adapters.glpi.pagination import (
    TicketFilters, PaginationParams, TicketStatus, 
    TicketPriority, DateRange, SortParams, SortOrder
)
...
class GLPIExample:
    ...
    async def example_1_basic_ticket_search(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Total: {response.total}, Página: {response.page}/{response.total_pages}")
            
            # Exibir alguns tickets
            for ticket in response.data[:3]:
                logger.info(f"Ticket #{ticket.get('id')}: {ticket.get('name')} - Status: {ticket.get('status')}")
    ...
    async def example_3_aggregated_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            weekly_metrics = await self.metrics_usecase.get_weekly_metrics(week_start=week_start)
            logger.info(f"Métricas semanais: {weekly_metrics.total_tickets} tickets")
            
            # Métricas mensais (N3)
            month_start = today.replace(day=1)
            monthly_metrics = await self.metrics_usecase.get_monthly_metrics(month=month_start)
    ...
    async def example_4_resilience_monitoring(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Circuit breaker: {summary.get('circuit_breaker_state', 'unknown')}")
    ...
    async def example_5_error_handling(self) -> Dict[str, Any]:
        ...
        logger.info(f"Resumo: {results['successful_operations']} sucessos, {results['failed_operations']} falhas")
        return results
