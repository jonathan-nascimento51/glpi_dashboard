...
class LevelMetrics(BaseModel):
    ...
    tempo_medio: PositiveFloat = Field(..., description="Tempo m?dio de resolu??o em horas")
...
class MetricsData(BaseModel):
    ...
    def validate_totals(self):
        ...
        if self.total_geral != calculated_total:
            raise ValueError(f'Total geral ({self.total_geral}) deve ser igual ? soma dos n?veis ({calculated_total})')
        return self
...
class Ticket(BaseModel):
    ...
    titulo: str = Field(..., min_length=1, max_length=255, description="T?tulo do ticket")
    ...
    data_atualizacao: Optional[datetime] = Field(None, description="Data de atualiza??o")
    data_resolucao: Optional[datetime] = Field(None, description="Data de resolu??o")
    tempo_resolucao: Optional[PositiveFloat] = Field(None, description="Tempo de resolu??o em horas")
    ...
    def validate_dates(self):
        if self.data_atualizacao and self.data_atualizacao < self.data_criacao:
            raise ValueError('Data de atualiza??o n?o pode ser anterior ? data de cria??o')
        if self.data_resolucao and self.data_resolucao < self.data_criacao:
            raise ValueError('Data de resolu??o n?o pode ser anterior ? data de cria??o')
        if self.status in [TicketStatus.RESOLVIDO, TicketStatus.FECHADO] and not self.data_resolucao:
            raise ValueError('Tickets resolvidos ou fechados devem ter data de resolu??o')
        return self
...
class FilterParams(BaseModel):
    ...
    prioridade: Optional[List[Priority]] = Field(None, description="Filtrar por prioridade")
    categoria: Optional[str] = Field(None, min_length=1, description="Filtrar por categoria")
    tecnico: Optional[str] = Field(None, min_length=1, description="Filtrar por t?cnico")
    data_inicio: Optional[datetime] = Field(None, description="Data de in?cio do per?odo")
