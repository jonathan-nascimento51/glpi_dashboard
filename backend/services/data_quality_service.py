...
class DataQualityReport:
    ...
    def is_healthy(self) -> bool:
        ...
        return self.overall_quality in [DataQualityLevel.EXCELLENT, DataQualityLevel.GOOD]
    
    @property
    def has_critical_issues(self) -> bool:
        """Retorna True se há problemas críticos."""
        return any(issue.severity in [DataQualityLevel.CRITICAL, DataQualityLevel.FAILED] 
...
class DataQualityService:
    ...
    def validate_data_quality(self, data: Dict[str, Any]) -> DataQualityReport:
        ...
        self.logger.info(f"Validação concluída. Qualidade: {overall_quality.value}, "
        ...
        try:
            ...
            for i, value in enumerate(numeric_values):
                ...
                if z_score > self._anomaly_threshold:
                    severity = DataQualityLevel.WARNING if z_score < 5.0 else DataQualityLevel.CRITICAL
                    issues.append(DataQualityIssue(
                        type="anomaly",
                        severity=severity,
    ...
    def _validate_temporal_consistency(self, data: Dict[str, Any]) -> List[DataQualityIssue]:
    ...
    def _validate_data_integrity(self, data: Dict[str, Any]) -> List[DataQualityIssue]:
        ...
        if all(field in data for field in required_fields):
            try:
                calculated_total = data["novos"] + data["pendentes"] + data["progresso"] + data["resolvidos"]
                ...
                if calculated_total != actual_total:
                    issues.append(DataQualityIssue(
                        ...
                        value={"calculated": calculated_total, "actual": actual_total}
                    ))
    ...
    def _calculate_quality_metrics(self, data: Dict[str, Any], issues: List[DataQualityIssue]) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "warning_issues": len([i for i in issues if i.severity == DataQualityLevel.WARNING]),
    ...
    def _determine_overall_quality(self, issues: List[DataQualityIssue]) -> DataQualityLevel:
        ...
        critical_count = len([i for i in issues if i.severity == DataQualityLevel.CRITICAL])
        warning_count = len([i for i in issues if i.severity == DataQualityLevel.WARNING])
