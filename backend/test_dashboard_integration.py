...
def test_dashboard_integration():
    ...
    try:
        ...
        if isinstance(metrics_by_level, dict):
            ...
            total_tickets = sum(
                level_data.get("total", 0) for level_data in metrics_by_level.values() if isinstance(level_data, dict)
            )
        ...
        if isinstance(general_metrics, dict):
            if "data" in general_metrics:
                ...
                if "metrics_by_level" in data:
                    ...
                    total_from_general = sum(
                        level.get("total", 0) for level in level_metrics.values() if isinstance(level, dict)
                    )
        ...
        if isinstance(metrics_by_level, dict):
            total_direct = sum(
                sum(status_data.values())
                for level_data in metrics_by_level.values()
                if isinstance(level_data, dict)
                for status_data in [level_data]
