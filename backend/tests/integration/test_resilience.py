...
from adapters.glpi.pagination import ServiceLevel, PaginationParams
...
def glpi_config():
    ...
    return GLPIClientConfig(
        ...
        retry_config=RetryConfig(
            max_attempts=3, min_wait=0.1, max_wait=1.0, exponential_base=2.0
        ),
...
class TestCircuitBreaker:
    ...
    async def test_circuit_breaker_opens_after_failures(
        self, resilient_client, mock_session
    ):
...
class TestSessionManagement:
    ...
    async def test_session_timeout_handling(self, resilient_client):
        ...
        resilient_client.session_manager.session_expires_at = (
    ...
    async def test_concurrent_session_renewal(self, resilient_client, mock_session):
        ...
        tasks = [resilient_client.search_tickets({}) for _ in range(3)]
...
class TestRateLimitHandling:
    ...
    async def test_rate_limit_429_handling(self, resilient_client, mock_session):
        ...
        mock_session.add_response(
            429, {"error": "Too Many Requests"}, {"Retry-After": "1"}
        )
        mock_session.add_response(200, {"data": []})
...
class TestPaginationResilience:
    ...
    @pytest.mark.asyncio
    async def test_long_pagination_with_intermittent_failures(
        self, resilient_client, mock_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_pagination_circuit_breaker_recovery(
        self, resilient_client, mock_session
    ):
...
class TestAggregatedMetricsResilience:
    ...
    @pytest.mark.asyncio
    async def test_partial_failure_in_metrics_aggregation(
        self, metrics_use_case, mock_session
    ):
        ...
        responses = [
            ...
            (
                200,
                {"data": [{"id": i} for i in range(11, 16)]},
            ),  # PROCESSANDO_ATRIBUIDO
            (
                200,
                {"data": [{"id": i} for i in range(16, 21)]},
    ...
    @pytest.mark.asyncio
    async def test_metrics_with_circuit_breaker_recovery(
        self, metrics_use_case, mock_session
    ):
...
class TestPerformanceResilience:
    ...
    async def test_high_concurrency_resilience(self, resilient_client, mock_session):
        ...
        tasks = [resilient_client.search_tickets({}) for _ in range(50)]
    ...
    async def test_memory_efficiency_long_running(self, resilient_client, mock_session):
        ...
        for page in range(100):
            if page < 50:
                mock_session.add_response(
                    200,
                    {"data": [{"id": i} for i in range(page * 10, (page + 1) * 10)]},
                )
            else:
