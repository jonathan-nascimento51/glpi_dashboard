...
class TestLoadTesting:
    ...
    def test_light_load_100_requests(self, mock_glpi_service):
        ...
        assert avg_response_time < 1.0, f"Tempo médio de resposta muito alto: {avg_response_time:.2f}s"
        assert max_response_time < 3.0, f"Tempo máximo de resposta muito alto: {max_response_time:.2f}s"
        assert p95_response_time < 2.0, f"95º percentil muito alto: {p95_response_time:.2f}s"
    ...
    def test_medium_load_concurrent_requests(self, mock_glpi_service):
        ...
        if response_times:
            ...
            assert max_response_time < 5.0, f"Tempo máximo de resposta muito alto: {max_response_time:.2f}s"
    ...
    def test_heavy_load_stress_test(self, mock_glpi_service):
        ...
        success_rate = len(successful_requests) / num_requests
        assert success_rate >= 0.80, f"Taxa de sucesso muito baixa para teste de stress: {success_rate:.2%}"
        ...
        if response_times:
            ...
            assert avg_response_time < 5.0, f"Tempo médio de resposta extremamente alto: {avg_response_time:.2f}s"
            assert max_response_time < 10.0, f"Tempo máximo de resposta extremamente alto: {max_response_time:.2f}s"
    ...
    def test_sustained_load_endurance(self, mock_glpi_service):
        ...
        assert success_rate >= 0.95, f"Taxa de sucesso degradou durante teste prolongado: {success_rate:.2%}"
        
        if response_times:
            overall_avg = statistics.mean(response_times)
            assert overall_avg < 2.0, f"Tempo médio de resposta degradou: {overall_avg:.2f}s"
        ...
        if len(window_avg_times) > 1:
            ...
            assert time_variance < 1.0, f"Performance muito instável ao longo do tempo: {time_variance:.3f}"
        ...
        if window_avg_times:
            ...
            print(f"   Variância entre janelas: {statistics.variance(window_avg_times):.3f}")
    ...
    def test_memory_leak_detection(self, mock_glpi_service):
        ...
        # Verificações
        assert total_memory_increase < 50, f"Possível vazamento de memória: {total_memory_increase:.2f}MB de aumento total"
        assert avg_memory_increase < 5, f"Aumento médio de memória muito alto: {avg_memory_increase:.2f}MB por iteração"
        ...
        if first_half and second_half:
            ...
            assert second_half_avg <= first_half_avg * 2, "Possível vazamento de memória detectado"
