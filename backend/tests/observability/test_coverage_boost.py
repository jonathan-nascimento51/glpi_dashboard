...
class TestSetupObservability:
    ...
    def test_setup_observability_custom_params(
        ...
        mock_logger.info.assert_called_once_with(
            "Sistema de observabilidade configurado", log_level="DEBUG", json_logs=False
        )
        assert result == app
...
class TestCreateErrorResponse:
    ...
    def test_create_error_response_with_api_error(self):
        ...
        error = BusinessError(message="Erro de negócio", details={"field": "value"})
...
class TestExceptionClasses:
    ...
    def test_external_service_error_with_service_name(self):
        ...
        error = ExternalServiceError(service="GLPI", message="Service unavailable")
    ...
    def test_validation_error_with_field_errors(self):
        ...
        assert (
            error.message == "Validation error on field 'email': Invalid email format"
        )
