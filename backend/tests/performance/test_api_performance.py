...
from tests.conftest import sample_metrics_data
...
class TestAPIPerformance:
    ...
    def test_concurrent_requests_performance(self, mock_glpi_service):
        ...
        )  # Nenhuma requisição deve demorar mais que 2 segundos
        assert (
    ...
    def test_database_query_performance(self, mock_glpi_service):
        ...
        with patch.object(
            mock_glpi_service, "get_dashboard_metrics", side_effect=slow_query
        ):
            start_time = time.time()
    ...
    def test_large_dataset_performance(self, mock_glpi_service):
        ...
        large_dataset = {
            ...
            "monthly_trends": [
                {"month": f"2024-{i:02d}", "tickets": i * 100} for i in range(1, 13)
            ],
        }

        with patch.object(
            mock_glpi_service, "get_dashboard_metrics", return_value=large_dataset
        ):
            start_time = time.time()
            ...
            assert (
                response_time < 2.0
            )  # Deve processar dataset grande em menos de 2 segundos
    ...
    def test_multiple_endpoints_performance(self, mock_glpi_service):
        ...
        endpoints = ["/dashboard/metrics", "/health", "/health/glpi"]
        ...
        for endpoint in endpoints:
            ...
            assert response.status_code in [
                200,
                500,
            ]  # Alguns podem falhar dependendo do mock
