...
class TestGetTicketCount:
    ...
    def test_get_ticket_count_success(self, mock_requests, mock_authenticated_service):
        ...
        mock_authenticated_service._make_authenticated_request = Mock(
    ...
    @patch("services.glpi_service.requests")
    def test_get_ticket_count_with_date_filter(
        self, mock_requests, mock_authenticated_service
    ):
...
class TestGetMetricsByLevelInternal:
    ...
    def test_get_metrics_by_level_internal_success(self, mock_authenticated_service):
        ...
        with patch.object(
            service, "get_ticket_count", side_effect=mock_get_ticket_count
        ):
            # Act
    ...
    def test_get_metrics_by_level_internal_with_date_filter(
        ...
        with patch.object(
            service, "get_ticket_count", return_value=5
        ) as mock_get_count:
            # Act
    ...
    def test_get_metrics_by_level_internal_ticket_count_failure(
        self, mock_authenticated_service
    ):
    ...
    def test_get_metrics_by_level_internal_partial_failure(
        self, mock_authenticated_service
    ):
        ...
        with patch.object(
            ...
            assert all(
                result["N1"][status] == 5 for status in service.status_map.keys()
            )
            assert all(
                result["N3"][status] == 5 for status in service.status_map.keys()
            )
            assert all(
                result["N4"][status] == 5 for status in service.status_map.keys()
            )
            # N2 deve ter valores 0 (falha)
            assert all(
                result["N2"][status] == 0 for status in service.status_map.keys()
            )
...
class TestGetDashboardMetricsWithDateFilter:
    ...
    def test_get_dashboard_metrics_with_date_filter_success(
        self, mock_authenticated_service
    ):
        ...
        with patch.object(
            service, "_get_metrics_by_level_internal", return_value=mock_level_metrics
        ):
            with patch.object(
                service,
                ...
                with patch.object(service, "_ensure_authenticated", return_value=True):
                    with patch.object(service, "discover_field_ids", return_value=True):
                        ...
                        assert (
                            result["data"]["filtros_aplicados"]["data_inicio"]
                            == start_date
                        )
                        assert (
                            result["data"]["filtros_aplicados"]["data_fim"] == end_date
                        )
    ...
    def test_get_dashboard_metrics_with_date_filter_cache_hit(
        self, mock_authenticated_service
    ):
    ...
    def test_get_dashboard_metrics_with_date_filter_authentication_failure(
        self, mock_authenticated_service
    ):
        """Testa falha de autenticação"""
        # Mock falha de autenticação
    ...
    def test_get_dashboard_metrics_with_date_filter_field_ids_failure(
        self, glpi_service
    ):
        ...
        with patch.object(service, "_ensure_authenticated", return_value=True):
            with patch.object(
                service, "discover_field_ids", return_value=False
            ) as mock_discover:
                result = service.get_dashboard_metrics_with_date_filter(
                    "2024-01-01", "2024-01-31"
                )
    ...
    def test_get_dashboard_metrics_with_date_filter_exception(
        self, mock_authenticated_service
    ):
        """Testa tratamento de exceções"""
        # Mock para simular exceção durante processamento
        mock_authenticated_service._get_general_metrics_internal = Mock(
    ...
    def test_get_dashboard_metrics_with_date_filter_no_dates(
        self, mock_authenticated_service
    ):
    ...
    def test_get_dashboard_metrics_with_date_filter_cache_storage(
        self, mock_authenticated_service
    ):
        ...
        with patch.object(service, "_is_cache_valid", return_value=False):
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        service, "_ensure_authenticated", return_value=True
                    ):
                        with patch.object(
                            service, "discover_field_ids", return_value=True
                        ):
                            with patch.object(
                                service, "_set_cache_data"
                            ) as mock_set_cache:
                                # Act
                                result = service.get_dashboard_metrics_with_date_filter(
                                    start_date, end_date
...
class TestTokenManagement:
    ...
    def test_ensure_authenticated_with_expired_token(self, glpi_service):
        ...
        with patch.object(
            service, "_authenticate_with_retry", return_value=True
        ) as mock_auth:
            # Act
    ...
    def test_ensure_authenticated_with_valid_token(self, mock_authenticated_service):
        ...
        mock_authenticated_service.token_created_at = (
            time.time() - 1800
        )  # 30 minutos atrás
...
class TestErrorHandling:
    ...
    def test_authentication_retry_mechanism(self, glpi_service):
        ...
        with patch.object(
            service, "_perform_authentication", side_effect=[False, False, True]
        ):
