...
class TestServiceLevelsConfig:
    ...
    def test_service_levels_have_unique_group_ids(self):
        ...
        assert len(group_ids) == len(unique_group_ids), (
            f"Group IDs duplicados encontrados: {group_ids}. " f"Cada nível deve ter um group_id único."
        )
    ...
    def test_service_levels_group_ids_are_correct(self):
        ...
        expected_group_ids = {"N1": 89, "N2": 90, "N3": 91, "N4": 92}
        ...
        for level, expected_group_id in expected_group_ids.items():
            ...
            assert actual_group_id == expected_group_id, (
                f"Nível {level} tem group_id {actual_group_id}, " f"mas deveria ter {expected_group_id}"
            )
    ...
    def test_service_levels_names_are_correct(self):
        ...
        for expected_level in expected_levels:
            assert expected_level in actual_levels, (
                f"Nível '{expected_level}' não encontrado. " f"Níveis disponíveis: {actual_levels}"
            )
    ...
    def test_service_levels_consistency_with_tests(self):
        ...
        assert len(service.service_levels) == 4, f"Esperado 4 níveis, mas encontrado {len(service.service_levels)}"
        ...
        assert group_ids == expected_sequence, f"Group IDs devem ser {expected_sequence}, mas encontrado {group_ids}"
