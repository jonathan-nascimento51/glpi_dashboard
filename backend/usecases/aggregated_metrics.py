...
class AggregatedMetricsUseCase:
    ...
    async def get_metrics_by_period(
        ...
        for level in service_levels:
            try:
                ...
                self.logger.error(
                    f"Erro ao obter métricas para nível {level.name}: {e}"
                )
    ...
    async def get_daily_metrics(
        ...
        return await self.get_metrics_by_period(
            MetricPeriod.DAILY, target_date, target_date, service_levels
        )
    ...
    async def get_weekly_metrics(
        ...
        return await self.get_metrics_by_period(
            MetricPeriod.WEEKLY, week_start, week_end, service_levels
        )
    ...
    async def get_monthly_metrics(
        ...
        return await self.get_metrics_by_period(
            MetricPeriod.MONTHLY, month_start, month_end, service_levels
        )
    ...
    async def get_comparative_metrics(
        ...
        for i in range(periods_back + 1):
            if current_period == MetricPeriod.DAILY:
                ...
                metrics = await self.get_monthly_metrics(
                    target_year, target_month, service_levels
                )
                period_key = f"month_{i}" if i > 0 else "current"

            else:
                raise ValueError(
                    f"Período {current_period} não suportado para comparação"
                )
    ...
    async def _get_service_level_metrics(
        self, service_level: ServiceLevel, start_date: date, end_date: date
    ) -> ServiceLevelMetric:
    ...
    async def _count_tickets_with_filters(self, filters: TicketFilters) -> int:
        ...
        try:
            ...
            query_params = (
                query_builder.add_filters(filters).add_pagination(pagination).build()
            )
    ...
    async def _get_backlog_count(
        self, service_level: ServiceLevel, period_start: date
    ) -> int:
        ...
        try:
            ...
            filters = TicketFilters(
                ...
                date_range=DateRange(
                    start_date=None, end_date=period_start - timedelta(days=1)
                ),
            )
        ...
        if period == MetricPeriod.DAILY:
            ...
            if not end_date:
                ...
                if quarter == 4:
                    ...
                    end_date = date(start_date.year, quarter * 3 + 1, 1) - timedelta(
                        days=1
                    )
            return start_date, end_date
...
# Factory functions
def create_aggregated_metrics_use_case(
    glpi_client: GLPIResilientClient,
) -> AggregatedMetricsUseCase:
...
def calculate_trend(
    ...
    for level in current_metrics.metrics_by_level.keys():
        ...
        previous_value = (
            getattr(previous_metrics.metrics_by_level.get(level), metric_name, 0)
            if previous_metrics.metrics_by_level.get(level)
            else 0
        )
