...
class GLPIMetrics:
    ...
    def _init_prometheus_metrics(self):
        ...
        self.info = Info(
            "glpi_client_info", "Information about GLPI client", registry=self.registry
        )
        ...
        if self.enable_prometheus:
            ...
            if event.duration_seconds is not None:
                self.request_duration.labels(
                    operation=event.operation.value, status=event.status
    ...
    def record_circuit_breaker_state(self, operation: OperationType, state: str):
        ...
        if self.enable_prometheus:
            ...
            self.circuit_breaker_state.labels(operation=operation.value).set(
                state_value
            )
    ...
    def record_circuit_breaker_failure(self, operation: OperationType):
        ...
        if self.enable_prometheus:
            self.circuit_breaker_failures.labels(operation=operation.value).inc()
    ...
    def record_retry(self, operation: OperationType, reason: str, delay_seconds: float):
        ...
        if self.enable_prometheus:
            self.retries_total.labels(
                operation=operation.value, retry_reason=reason
            ).inc()

            self.retry_delay.labels(operation=operation.value).observe(delay_seconds)
...
def measure_operation(
    operation: OperationType, labels: Optional[Dict[str, str]] = None
):
