...
from schemas.dashboard import DashboardMetrics, LevelMetrics, NiveisMetrics, TendenciasMetrics, TechnicianRanking, NewTicket, ApiResponse
...
class MockDataGenerator:
    ...
    def __init__(self):
        self.technician_names = [
            ...
            "Carlos Ferreira", "Lucia Almeida", "Rafael Souza", "Fernanda Lima",
            "Gabriel Mendes", "Juliana Rocha", "Bruno Cardoso", "Patricia Dias"
        ]
        
        self.categories = [
            "Hardware", "Software", "Rede", "Impressora", "Sistema", 
            "Email", "Telefone", "Acesso", "Backup", "Segurança"
        ]
    ...
    def generate_dashboard_metrics(self, filters: Dict[str, Any] = None) -> DashboardMetrics:
        """Gera métricas mock para o dashboard"""
        
        # Gerar números realistas baseados em um cenário típico
        base_novos = random.randint(15, 45)
        base_pendentes = random.randint(30, 80)
        base_progresso = random.randint(20, 60)
        base_resolvidos = random.randint(100, 300)
        
        # Métricas por nível (N1 tem mais tickets, N4 tem menos)
        n1_metrics = LevelMetrics(
            novos=int(base_novos * 0.5),
            pendentes=int(base_pendentes * 0.4),
            progresso=int(base_progresso * 0.5),
            resolvidos=int(base_resolvidos * 0.6)
        )
        
        n2_metrics = LevelMetrics(
            novos=int(base_novos * 0.3),
            pendentes=int(base_pendentes * 0.35),
            progresso=int(base_progresso * 0.3),
            resolvidos=int(base_resolvidos * 0.25)
        )
        
        n3_metrics = LevelMetrics(
            novos=int(base_novos * 0.15),
            pendentes=int(base_pendentes * 0.2),
            progresso=int(base_progresso * 0.15),
            resolvidos=int(base_resolvidos * 0.12)
        )
        
        n4_metrics = LevelMetrics(
            novos=int(base_novos * 0.05),
            pendentes=int(base_pendentes * 0.05),
            progresso=int(base_progresso * 0.05),
            resolvidos=int(base_resolvidos * 0.03)
        )
    ...
    def generate_technician_ranking(self, limit: int = 10) -> List[TechnicianRanking]:
        """Gera ranking mock de técnicos"""
        
        technicians = []
        levels = ["N1", "N2", "N3", "N4"]
        
        for i in range(min(limit, len(self.technician_names))):
            name = self.technician_names[i]
            level = random.choice(levels)
            
            # N1 resolve mais tickets, N4 resolve menos mas mais complexos
            if level == "N1":
                ticket_count = random.randint(45, 85)
                performance_score = random.uniform(3.8, 4.5)
            elif level == "N2":
                ticket_count = random.randint(30, 55)
                performance_score = random.uniform(4.0, 4.7)
            elif level == "N3":
                ticket_count = random.randint(15, 35)
                performance_score = random.uniform(4.2, 4.8)
            else:  # N4
                ticket_count = random.randint(8, 20)
                performance_score = random.uniform(4.5, 5.0)
            
            technicians.append(TechnicianRanking(
                id=i + 1,
                name=name,
...
def get_mock_dashboard_metrics(filters: Dict[str, Any] = None) -> DashboardMetrics:
