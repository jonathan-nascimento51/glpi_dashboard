    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        try:
            ...
            if not isinstance(record, logging.LogRecord):
                return json.dumps({"error": "Invalid log record", "timestamp": datetime.now().isoformat()})
            ...
            if hasattr(record, 'lineno') and record.lineno:
                log_entry["line_number"] = int(record.lineno)
            ...
            if record.exc_info:
                try:
                    ...
                    log_entry["exception"] = {"error": "Failed to format exception info"}
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if not isinstance(logger, StructuredLogger):
                    print(f"Warning: Invalid logger provided to log_api_call for {func.__name__}")
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                # Validar parâmetros
                if not isinstance(logger, StructuredLogger):
                    print(f"Warning: Invalid logger provided to log_performance for {func.__name__}")
                    return func(*args, **kwargs)
...
def log_api_response(logger: StructuredLogger, response_data: Any, 
    ...
    try:
        # Validar logger
        if not isinstance(logger, StructuredLogger):
            print("Warning: Invalid logger provided to log_api_response")
            return
        ...
        if response_time is not None:
            try:
                if isinstance(response_time, (int, float)) and response_time >= 0:
                    log_data["response_time_seconds"] = round(float(response_time), 4)
                else:
