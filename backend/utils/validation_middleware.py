...
class ValidationMiddleware:
    ...
    def _should_validate_response(self, request: Request, response: Response) -> bool:
        ...
        return (
            ...
            any(request.url.path.startswith(endpoint) for endpoint in api_endpoints) and
            response.headers.get('content-type', '').startswith('application/json')
        )
    
    async def _validate_response(self, request: Request, response: Response) -> Optional[Response]:
    ...
    def _get_fallback_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'data': {
                'niveis': {
                    'N1': {'total': 0, 'resolvidos': 0, 'pendentes': 0, 'tempo_medio': 1.0},
                    'N2': {'total': 0, 'resolvidos': 0, 'pendentes': 0, 'tempo_medio': 1.0},
                    'N3': {'total': 0, 'resolvidos': 0, 'pendentes': 0, 'tempo_medio': 1.0}
                },
                'total_tickets': 0,
    ...
    async def _create_error_response(self, message: str, status_code: int = 400) -> JSONResponse:
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            try:
                ...
                raise HTTPException(
                    status_code=500,
                    detail={'message': 'Erro interno do servidor'}
                )
