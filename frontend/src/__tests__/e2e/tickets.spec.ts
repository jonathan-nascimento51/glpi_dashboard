...
test.describe('Tickets E2E Tests', () => {
  ...
  test('should display tickets list', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/Tickets - GLPI Dashboard/);
  ...
  test('should search tickets by text', async ({ page }) => {
    ...
    if (resultCount > 0) {
      ...
      const title = await firstResult.locator('[data-testid="ticket-title"]').textContent();
      const description = await firstResult.locator('[data-testid="ticket-description"]').textContent();
  ...
  test('should open ticket details', async ({ page }) => {
    ...
    await expect(page.url()).toMatch(/\/tickets\/\d+/);
  ...
  test('should create new ticket', async ({ page }) => {
    ...
    await page.fill('[data-testid="ticket-description-input"]', 'Descrição do ticket criado via teste E2E');
  ...
  test('should be responsive on mobile devices', async ({ page }) => {
    ...
    await expect(ticketCards.first()).toHaveClass(/mobile-card/);
