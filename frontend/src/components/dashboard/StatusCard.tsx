...
export const StatusCard = memo<StatusCardProps>(function StatusCard({
  ...
  // Memoizar Ã­cones para evitar recÃ¡lculos
  ...
  return (
    ...
    >
      <Card 
        className="metric-card premium-shadow relative overflow-hidden rounded-xl"
      ...
      >
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 relative z-10">
          <CardTitle className="figma-subheading uppercase tracking-wide" data-testid="metric-title">
            {title}
          </CardTitle>
          {StatusIcon && (
            <motion.div
        ...
        <CardContent className="relative z-10">
          <div className="flex items-center justify-between">
            ...
            {status && (
              ...
              >
                <Badge 
                  className={cn("capitalize text-xs font-semibold px-3 py-1 border-0 shadow-lg text-white", `status-card-${status}`)}
                >
                  {status}
                </Badge>
              </motion.div>
