    ...
    try:
        ...
        for tech_id in technician_ids:
            ...
            try:
                ...
                technicians_by_level["Sem_Nivel"].append({"id": tech_id, "name": tech_name, "level": "ERRO", "tickets": 0})
        ...
        try:
            ...
            for i, tech in enumerate(ranking[:10]):  # Mostrar top 10
                print(
                    f"  {i+1}. {tech.get('name', 'N/A')} - Nível: {tech.get('level', 'N/A')} - Tickets: {tech.get('total', 0)}"
                )
        except Exception as e:
        ...
        for level, group_id in service.service_levels.items():
            try:
                ...
                if group_response and group_response.ok:
                    ...
                    print(
                        f"❌ {level} (ID {group_id}): Grupo não encontrado ou erro {group_response.status_code if group_response else 'Sem resposta'}"
                    )
            except Exception as e:
